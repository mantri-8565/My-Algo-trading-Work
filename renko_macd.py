...
def MACD(DF,a,b,c):
    df=DF.copy()
    df['high line']=df["Close"].ewm(span=a,min_periods=a).mean()
    df['low line']=df["Close"].ewm(span=b,min_periods=b).mean()
...
for ticker in tickers:
    print("Getting TI for",ticker)
    renko_data=RENKO(ohlcv_dict[ticker], 120)
    ohlcv_dict[ticker]['Date']=ohlcv[ticker].index 
    ...
    ohlcv_renko[ticker]["macd"]=MACD(ohlcv_renko[ticker], 12, 26, 9)[0]
    ohlcv_renko[ticker]["signal"]=MACD(ohlcv_renko[ticker], 12, 26, 9)[1]
    ohlcv_renko[ticker].dropna(inplace=True)
    ohlcv_renko[ticker].reset_index(inplace=True, drop=True)
    ohlcv_renko[ticker]["slope_macd"]=Slope(ohlcv_renko[ticker]["macd"], 5)    
    ohlcv_renko[ticker]["slope_signal"]=Slope(ohlcv_renko[ticker]["signal"], 5)
...
for ticker in tickers:
    ...
    for i in range(len(ohlcv_renko[ticker])):
        if ticker_signal[ticker] == "":
            ...
            if ohlcv_renko[ticker]["bar_num"][i]>=2 and ohlcv_renko[ticker]["macd"][i]>ohlcv_renko[ticker]["signal"][i] and \
                ohlcv_renko[ticker]['slope_macd'][i]>ohlcv_renko[ticker]['slope_signal'][i]:
                ticker_signal[ticker] = "Buy"
                
            elif ohlcv_renko[ticker]["bar_num"][i]<=-2 and ohlcv_renko[ticker]["macd"][i]<ohlcv_renko[ticker]["signal"][i] and \
                ohlcv_renko[ticker]['slope_macd'][i]<ohlcv_renko[ticker]['slope_signal'][i]:
            ...
            if ohlcv_renko[ticker]["bar_num"][i]<=-2 and ohlcv_renko[ticker]["macd"][i]<ohlcv_renko[ticker]["signal"][i] and \
                ohlcv_renko[ticker]['slope_macd'][i]<ohlcv_renko[ticker]['slope_signal'][i]:
                ticker_signal[ticker] = "Sell"
            elif  ohlcv_renko[ticker]["macd"][i]<ohlcv_renko[ticker]["signal"][i] and \
                ohlcv_renko[ticker]['slope_macd'][i]<ohlcv_renko[ticker]['slope_signal'][i]:
                ticker_signal[ticker] = ""        
